 ...
 * @author DELL
 */
public class main_doAn extends javax.swing.JFrame {
    ...
    public main_doAn(String username) {
        initComponents();
        txt_TaiKhoan.setText(username);

        tableModel = (DefaultTableModel) tblDoAn.getModel();
        showDoAn();
    }
    ...
    private void addDoAn() {
        if (txtMaDoAn.getText().trim().equals("")) {
            JOptionPane.showMessageDialog(this, "Mã đồ ăn không được trống");
            txtMaDoAn.requestFocus();
            txtMaDoAn.selectAll();
        } else if (txtTenDoAn.getText().equals("")) {
            JOptionPane.showMessageDialog(this, "Tên đồ ăn không được trống");
            txtTenDoAn.requestFocus();
            txtTenDoAn.selectAll();
        } else if (txtGia.getText().trim().equals("")) {
            JOptionPane.showMessageDialog(this, "Giá đồ ăn không được trống");
            txtGia.requestFocus();
            txtGia.selectAll();
        } else if (!txtGia.getText().trim().matches("^[1-9][0-9]*$")) {
            JOptionPane.showMessageDialog(this, "Giá đồ ăn phải > 0");
            txtGia.requestFocus();
            txtGia.selectAll();
        } else {
            conn = con.getConnection();
            try {
                String sql = "{call addDoAn (?, ?, ?)}";
                CallableStatement statement = conn.prepareCall(sql);

                statement.setString(1, txtMaDoAn.getText());
                statement.setString(2, txtTenDoAn.getText());
                statement.setInt(3, Integer.valueOf(txtGia.getText().trim()));

                ResultSet rs = statement.executeQuery();
                while (rs.next()) {
                    if (rs.getInt("ketqua") == -1) {
                        System.out.println("Mã đồ ăn đã tồn tại");
                        JOptionPane.showMessageDialog(this, "Mã đồ ăn đã tồn tại");
                    } else if (rs.getInt("ketqua") == 0) {
                        System.out.println("Giá món ăn phải dương");
                        JOptionPane.showMessageDialog(this, "Giá món ăn phải dương");
                    } else if (rs.getInt("ketqua") == 1) {
                        System.out.println("Thêm món ăn thành công");
                        JOptionPane.showMessageDialog(this, "Thêm đồ ăn thành công");
                    }
                }
            } catch (Exception e) {
            } finally {
                if (conn != null) {
                    try {
                        ...
                        Logger.getLogger(main_doAn.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
            }
        }
    ...
    private void saveUpdate() {
        if (txtMaDoAn.getText().trim().equals("")) {
            ...
            try {
                ...
                while (rs.next()) {
                    if (rs.getInt("ketqua") == -1) {
                        System.out.println("Mã đồ ăn không tồn tại");
                        JOptionPane.showMessageDialog(this, "Mã đồ ăn không tồn tại");
                    } else if (rs.getInt("ketqua") == 0) {
                        System.out.println("Giá món ăn phải dương");
                        JOptionPane.showMessageDialog(this, "Giá món ăn phải dương");
                    } else if (rs.getInt("ketqua") == 1) {
                ...
                .addGroup(jPanel4Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                    ...
                    .addGroup(javax.swing.GroupLayout.Alignment.LEADING, jPanel4Layout.createSequentialGroup()
                        ...
                        .addGroup(jPanel4Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                            .addComponent(txtTenDoAn, javax.swing.GroupLayout.DEFAULT_SIZE, 160, Short.MAX_VALUE)
    ...
    private void tblDoAnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblDoAnMouseClicked
        ...
        if (selectedIndex >= 0) {
            doAn std = list.get(selectedIndex);
            txtMaDoAn.setText(std.getMaDoAn());
            txtTenDoAn.setText(std.getTenDoAn());
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(main_doAn.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
